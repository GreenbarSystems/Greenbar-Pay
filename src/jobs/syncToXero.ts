/**
 * sync-to-xero — pushes approved invoices to Xero as ACCPAY Bills.
 *
 * Mirrors sync-to-qbo. Triggered by POST /api/ap/exports with format='xero'.
 */
...
export async function handleSyncToXero(
  job: PgBoss.Job<JobPayloads[typeof JOB.syncToXero]>,
): Promise<void> {
  ...
  if (!claimed) {
    console.warn(`[sync-to-xero] export=${exportId} not claimable`);
    return;
  }
  ...
  if (!conn) {
    await failExport(organizationId, exportId, "No active Xero connection found");
    return;
  }
  ...
  if (conn.tokenExpiresAt.getTime() - Date.now() < 5 * 60 * 1000) {
    try {
      const refreshed = await refreshXeroTokens(organizationId, conn.id, conn.refreshToken);
  ...
  for (const item of items) {
    ...
    try {
      const result = await createXeroBill(tenantId, accessToken, input);

      await withOrgAsWorker(organizationId, async (tx) => {
        await tx
          .update(exportItems)
          .set({ status: "synced", externalId: result.invoiceId })
          .where(eq(exportItems.id, item.itemId));
      });
      syncedCount++;
    } catch (err) {
      console.error(`[sync-to-xero] invoice=${item.invoiceId} failed:`, err);
      await withOrgAsWorker(organizationId, async (tx) => {
        await tx
          .update(exportItems)
  ...
  await withOrgAsWorker(organizationId, async (tx) => {
    if (allFailed) {
      await tx
        ...
        .set({
          status: "failed",
          errorMessage: `All ${failedCount} invoice(s) failed to sync to Xero`,
        })
        .where(
          and(
            eq(exportsTable.id, exportId),
    ...
    await tx.insert(auditEvents).values({
      organizationId,
      actorType: "worker",
      action: "export.xero_sync_completed",
      entityType: "export",
      entityId: exportId,
